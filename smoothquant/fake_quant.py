...
@torch.no_grad()
def quantize_weight_per_channel_absmax_4bit(w, n_bits=4):
    scales = w.abs().max(dim=-1, keepdim=True)[0]
    q_max = 2 ** (n_bits - 1) - 1
    scales.clamp_(min=1e-5).div_(q_max)
    w.div_(scales).round_().mul_(scales)
...
@torch.no_grad()
def quantize_weight_per_tensor_absmax_4bit(w, n_bits=4):
    scales = w.abs().max()
    q_max = 2 ** (n_bits - 1) - 1
    scales.clamp_(min=1e-5).div_(q_max)
    w.div_(scales).round_().mul_(scales)
...
@torch.no_grad()
def quantize_activation_per_token_absmax_4bit(t, n_bits=4):
    t_shape = t.shape
    t.view(-1, t_shape[-1])
    scales = t.abs().max(dim=-1, keepdim=True)[0]
    q_max = 2 ** (n_bits - 1) - 1
...
@torch.no_grad()
def quantize_activation_per_tensor_absmax_4bit(t, n_bits=4):
    t_shape = t.shape
    t.view(-1, t_shape[-1])
    scales = t.abs().max()
    q_max = 2 ** (n_bits - 1) - 1
...
class W4A4Linear(nn.Module):
    def __init__(
        ...
        if act_quant == "per_token":
            self.act_quant_name = "per_token"
            self.act_quant = partial(quantize_activation_per_token_absmax_4bit, n_bits=4)
        elif act_quant == "per_tensor":
            self.act_quant_name = "per_tensor"
            self.act_quant = partial(quantize_activation_per_tensor_absmax_4bit, n_bits=4)
        else:
            raise ValueError(f"Invalid act_quant: {act_quant}")
    ...
    def from_float(
        ...
        if weight_quant == "per_channel":
            new_module.weight = quantize_weight_per_channel_absmax_4bit(
                module.weight, n_bits=4
            )  # use 8-bit integer for weight
        elif weight_quant == "per_tensor":
            new_module.weight = quantize_weight_per_tensor_absmax_4bit(
                module.weight, n_bits=4
            )
        else:
            raise ValueError(f"Invalid weight_quant: {weight_quant}")
...
def quantize_attention_only(
    ...
    for name, m in model.model.named_modules():
        if isinstance(m, (LlamaAttention, MistralAttention)):
            # 這裡我們模擬通過量化 q_proj, k_proj, v_proj 的輸出來量化 BMM 輸入
            m.q_proj = W8A8Linear.from_float(
                m.q_proj,
                weight_quant=weight_quant,
                act_quant=act_quant,
...
def quantize_attention_only_4bit(
    model, weight_quant="per_channel", act_quant="per_token", quantize_bmm_input=False
):
    from transformers.models.llama.modeling_llama import (
        LlamaAttention,
...
def quantize_llama_like_layer(
    model, weight_quant="per_channel", act_quant="per_token", quantize_bmm_input=False, quant_layers=range(4)
):
    from transformers.models.llama.modeling_llama import (
        LlamaAttention,
        LlamaMLP,
    ...
    for name, m in model.model.named_modules():
        ...
        if layer_idx not in quant_layers:
            print(f"Skipping layer {layer_idx} for quantization")
            continue

        print(f"Quantizing layer {layer_idx}")

        # 對 LlamaMLP 和 MistralMLP 進行量化
        if isinstance(m, (LlamaMLP, MistralMLP)):
            m.gate_proj = W4A4Linear.from_float(
                m.gate_proj, weight_quant=weight_quant, act_quant=act_quant
            )
        ...
        # 對 LlamaAttention 和 MistralAttention 進行量化
        elif isinstance(m, (LlamaAttention, MistralAttention)):
            m.q_proj = W4A4Linear.from_float(
                m.q_proj,
                weight_quant=weight_quant,
